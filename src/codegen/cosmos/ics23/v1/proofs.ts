 ...
 * trusted root, these key and values must be in the referenced merkle tree. The
 * only possible issue is maliablity in LeafOp, such as providing extra prefix
 ...
 * produce the same value. With LengthOp this is tricker but not impossible. Which
 * is why the "leafPrefixEqual" field in the ProofSpec is valuable to prevent this
 * mutability. And why all trees should length-prefix the data before hashing it.
 */
 ...
 * in this case): hkey = prehashKey(key) hvalue = prehashValue(value) Then combine
 * the bytes, and hash it output = hash(prefix || length(hkey) || hkey ||
 ...
 * result. The result of the previous step is passed in, so the signature of this
 * op is: innerOp(child) -> output The result of applying InnerOp should be: output
 * = op.hash(op.prefix || child || op.suffix) where the || operator is
 * concatenation of binary data, and child is the result of hashing all the tree
 * below this step. Any special data, like prepending child with the length, or
 * prepending the entire operation with some value to differentiate from leaf
 * nodes, should be included in prefix and suffix. If either of prefix or suffix is
 * empty, we just treat it as an empty string
 */
 ...
 * from a given store are neighbors. This enables: isLeftMost(spec: InnerSpec, op:
 * InnerOp) isRightMost(spec: InnerSpec, op: InnerOp) isLeftNeighbor(spec:
 * InnerSpec, left: InnerOp, right: InnerOp)
 */
