...
export interface Msg {
  ...
  setGaslessContracts(request: MsgSetGaslessContracts): Promise<MsgSetGaslessContractsResponse>;
}
export class MsgClientImpl implements Msg {
  ...
  constructor(rpc: Rpc) {
    ...
    this.setGaslessContracts = this.setGaslessContracts.bind(this);
  }
  storeCode(request: MsgStoreCode): Promise<MsgStoreCodeResponse> {
  ...
  setGaslessContracts(request: MsgSetGaslessContracts): Promise<MsgSetGaslessContractsResponse> {
    const data = MsgSetGaslessContracts.encode(request).finish();
    const promise = this.rpc.request("cosmwasm.wasm.v1.Msg", "SetGaslessContracts", data);
    return promise.then(data => MsgSetGaslessContractsResponse.decode(new BinaryReader(data)));
  }
}